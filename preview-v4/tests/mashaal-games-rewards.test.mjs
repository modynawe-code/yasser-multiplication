...
test('Mashaal game learning plugs into the generic adapter without editing its core contract',async()=>{
  ...
  let mashaalSaves=0;
  const mashaalProvider=createMashaalGameLearningProvider({
    getState:()=>mashaal,
    saveState:()=>{mashaalSaves+=1;},
    random:()=>0,
    now:()=>12345
  });
  const adapter=createGameLearningAdapter({
    getYasserState:()=>yasser,
    getKhaledState:()=>khaled,
    additionalProviders:{mashaal:mashaalProvider},
    random:()=>0
  });
...
test('additional learner providers cannot replace established Yasser or Khaled providers',()=>{
  assert.throws(()=>createGameLearningAdapter({
    getYasserState:()=>createInitialState(),
    getKhaledState:()=>createInitialKhaledState(),
    additionalProviders:{yasser:{nextChallenge(){return{};}}}
  }),/already configured/);
});

test('Mashaal developmental catalog contains twelve learner-specific rewards with unique graphics',()=>{
  assert.equal(MASHAAL_REWARD_CATALOG.length,12);
  assert.equal(new Set(MASHAAL_REWARD_CATALOG.map(item=>item.id)).size,12);
  assert.equal(new Set(MASHAAL_REWARD_CATALOG.map(item=>item.graphicKey)).size,12);
  assert.ok(MASHAAL_REWARD_CATALOG.every(item=>item.id.startsWith('mashaal-')));
});

test('game events unlock Mashaal rewards through shared ledger and rules without leaking to another learner',()=>{
  const catalogRegistry=createRewardCatalogRegistry();
  catalogRegistry.register(MASHAAL_REWARD_CATALOG_ID,MASHAAL_REWARD_CATALOG);
  const ruleRegistry=createRewardRuleRegistry();
  ruleRegistry.register('mashaal',{mode:'developmental',rules:MASHAAL_REWARD_RULES});
  ...
  const event=createGameEvent({
    type:'game.completed',gameId:'rock-paper-scissors',learnerId:'mashaal',sessionId:'match-1',
    at:'2026-09-10T16:30:00.000Z',payload:{firstCompletion:true}
  });
  ...
  const duplicate=rewards.handle(event);
  ...
  assert.equal(rewards.handle(other).added,0);
...
test('Mashaal encouragement is a presentation pack over the shared resolver',()=>{
  ...
  registry.register('mashaal',MASHAAL_ENCOURAGEMENT_PACK);
