...
test('approved source alone does not open recitation until a verified local offline asset is present',()=>{
...
test('recitation manifest rejects missing integrity data, remote paths and non-human audio',()=>{
  const base={id:'recitation-001',sourceId:'kfgqpc-ibrahim-al-akhdar-hafs',localPath:'./assets/recitation/001.mp3',sha256:'a'.repeat(64),surahNumber:1,surahNameAr:'الفاتحة',humanVoice:true};
  ...
  assert.equal(validateMashaalRecitationAsset({...base,localPath:'https://example.com/001.mp3'}).valid,false);
