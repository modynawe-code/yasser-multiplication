...
test('all reviewed KG3 skills have starter activity content',()=>{
...
test('Quran recitation stays closed until approved human recitation audio is available',()=>{
  const plan=createMashaalActivityPlan('listen-repeat');assert.equal(plan.contentReady,false);assert.deepEqual(plan.activities,[]);
});
