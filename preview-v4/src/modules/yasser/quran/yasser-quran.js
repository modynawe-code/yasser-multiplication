...
const TERM_1=Object.freeze({
  ...
  memorization:Object.freeze([[68,'القلم',564]])
});
const TERM_2=Object.freeze({
  recitation:Object.freeze([
    [34,'سبأ',428],[33,'الأحزاب',418],[32,'السجدة',415]
  ]),
  memorization:Object.freeze([[67,'الملك',562]])
});
...
let mounted=false,player=null,term=1,mode='recitation',selected=null,repeatTarget=3,repeatCount=0;
...
function keyFor(item){return `${item.surahNumber}`;}
function levelFor(item){return loadProgress()?.levels?.[keyFor(item)]||LEVELS[0];}
function setLevel(item,level){
  const current=loadProgress();
  const levels={...(current.levels||{}),[keyFor(item)]:level};
  saveProgress({...current,levels,last:{surahNumber:item.surahNumber,surahNameAr:item.surahNameAr,level,term,updatedAt:new Date().toISOString()}});
  renderLast();
}
...
function ensureShell(){
  ...
  view.innerHTML=`<div class="yasser-quran-shell">
    ...
    <div class="yasser-quran-modes" role="group" aria-label="نوع الدراسة">
      ...
      <button type="button" data-yq-mode="memorization" aria-pressed="false"><strong>الحفظ غيب</strong><span>أتدرّب ثم أخفي المصحف</span></button>
    </div>
    ...
    <section class="yasser-quran-stage" id="yasserQuranStage" hidden>
      ...
      <p class="yasser-quran-memory-hint" id="yasserQuranMemoryHint" hidden>في وضع الغيب تختفي صفحة المصحف. اقرأ من حفظك، ثم اضغط «أظهر المصحف» للمراجعة.</p>
    </section>
...
function renderLast(){
  ...
  host.textContent=last?.surahNameAr?`آخر حفظ: سورة ${last.surahNameAr} • ${last.level}`:'ابدأ أول سورة وسنحفظ آخر موضع لك.';
}
function renderTabs(){
...
function renderList(){
  ...
  items.forEach(item=>{
    ...
    const badge=mode==='memorization'?`<span class="yasser-quran-level">${levelFor(item)}</span>`:`<span class="yasser-quran-page">صفحة ${item.pageNumber}</span>`;
    button.innerHTML=`<span class="yasser-quran-number">${item.surahNumber}</span><span class="yasser-quran-name"><strong>سورة ${item.surahNameAr}</strong><small>${mode==='memorization'?'حفظ غيب':'تلاوة'}</small></span>${badge}`;
    button.addEventListener('click',()=>openSurah(item,button));host.appendChild(button);
  });
}
function resetStage(){
  selected=null;repeatCount=0;destroyPlayer();
...
function updateLevelButton(){
  const button=document.getElementById('yasserQuranLevel');if(!button||!selected)return;
  button.textContent=`الحالة: ${levelFor(selected)}`;
}
function cycleLevel(){
  if(!selected)return;const current=levelFor(selected);const next=LEVELS[(LEVELS.indexOf(current)+1)%LEVELS.length];setLevel(selected,next);updateLevelButton();renderList();
}
...
function openSurah(item,button){
  destroyPlayer();selected=item;repeatCount=0;
  ...
  document.getElementById('yasserQuranStageMode').textContent=mode==='memorization'?'الحفظ غيب':'التلاوة';
  ...
  player=mountQuranSurahPlayer(document.getElementById('yasserQuranPlayerHost'),{
    surahNameAr:item.surahNameAr,surahNumber:item.surahNumber,audioPath:item.audioPath,mushafPage:item.mushafPage,retryPlayText:'اضغط تشغيل مرة ثانية',
    onCompleted:()=>{
      if(mode!=='memorization'||repeatTarget<=1)return;
      repeatCount+=1;
      if(repeatCount<repeatTarget)setTimeout(()=>{void player?.restart?.();},350);
    }
  });
