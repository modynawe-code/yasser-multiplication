...
function renderStimulus(model){
  const host=byId('mashaalActivityStimulus');if(!host)return;host.innerHTML='';
  const stimulus=model?.stimulus||{};
...
export function createMashaalController({repository,onExitToHub}={}){
  ...
  function renderSkills(domainId){
    const grid=byId('mashaalSkillGrid');if(!grid)return;
    grid.innerHTML='';
    for(const skill of getMashaalDomainSkills(domainId)){
      ...
      const title=document.createElement('strong');title.textContent=skill.title;
      const stateLabel=document.createElement('span');stateLabel.textContent=skill.contentReady?'ابدئي ✨':'قريبًا';
      button.append(title,stateLabel);
      if(!skill.contentReady){button.disabled=true;button.setAttribute('aria-disabled','true');}
  ...
  function openDomain(domainId){
    ...
    byId('mashaalDomainMessage').textContent='اختاري لعبة نبدأ فيها.';
  ...
  function bind(){
    ...
    byId('mashaalHearActivity')?.addEventListener('click',()=>currentViewModel&&speech.speak(currentViewModel.audioPromptAr));
    byId('mashaalActivityCheck')?.addEventListener('click',()=>selectedChoices.size&&submitAnswer([...selectedChoices]));
    byId('mashaalDomainBack')?.addEventListener('click',backHome);
    byId('mashaalActivityBack')?.addEventListener('click',backDomain);
    byId('mashaalToHub')?.addEventListener('click',exit);
    byId('mashaalDomainToHub')?.addEventListener('click',exit);
    byId('mashaalActivityToHub')?.addEventListener('click',exit);
  }
