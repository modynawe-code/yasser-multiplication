...
export function submitRpsChoice(state,{playerId,choice}={}){
  ...
  if(next.chooserIndex===0){next.chooserIndex=1;return{ok:true,state:next,reveal:false};}
...
export function nextRpsRound(state){
  if(!['revealed'].includes(state?.status))return{ok:false,reason:'round-not-revealed'};
  const next=clone(state);next.round+=1;next.status='choosing';next.chooserIndex=0;next.choices={};next.roundWinner=null;return{ok:true,state:next};
}
