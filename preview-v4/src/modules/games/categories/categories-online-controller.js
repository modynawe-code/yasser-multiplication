...
export function createCategoriesOnlineController({showView,onBack,roomClient=createGameRoomClient()}={}){
  ...
  function duplicateCount(cat,answer){const key=normalizeArabicAnswer(answer);if(!key)return 0;let count=0;for(const pid of s().players){const other=normalizeArabicAnswer(s().submissions?.[pid]?.answers?.[cat]||'');if(other===key)count++;}return count;}
  ...
  function recordHistory(){const state=s(),key=`${room.code}:${state.round}:${JSON.stringify(state.scores)}`;if(historyKey===key)return;historyKey=key;const winnerLearners=(state.winnerIds||[]).map(pid=>learner(pid)),winnerNames=(state.winnerIds||[]).map(pid=>name(pid)),totals=Object.fromEntries(state.players.map(pid=>[learner(pid),Number(state.scores?.[pid]||0)]));safeWriteHistory([{id:`online-${room.code}-${Date.now()}`,day:localDayKey(),createdAt:new Date().toISOString(),winnerIds:winnerLearners,winnerNames,rounds:state.roundsTotal,totals},...safeReadHistory()]);}
  function renderResult(){stopTimer();showPanel('fwcoResult');syncMini();const state=s(),finished=state.status==='finished',winners=(finished?state.winnerIds:state.roundWinnerIds)||[],names=winners.map(name);byId('fwcoResultKicker').textContent=finished?'النتيجة النهائية':`نتيجة الجولة ${state.round}`;byId('fwcoResultTitle').textContent=names.length>1?`تعادل ${names.join(' و ')} 🤝`:`${names[0]||'أحسنتم'} ${finished?'بطل المباراة 🏆':'فاز بالجولة ⭐'}`;renderScoreboard();byId('fwcoNext').hidden=finished||!isHost();byId('fwcoReset').hidden=!finished||!isHost();if(finished)recordHistory();}
  ...
  async function reset(){if(busy)return;busy=true;historyKey='';timeoutKey='';try{await session.submit('reset');}catch(e){handleError(e);}finally{busy=false;}}
