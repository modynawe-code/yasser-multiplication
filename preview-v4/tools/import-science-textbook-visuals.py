...
# PDF pages are 1-based. Each asset is deliberately restricted to its lesson range.
TARGETS = {
...
def local_window(image,match,target):
    ...
    center=ty+th/2+target.get("bias",0)
    height=min(int(target.get("window",1100)),image.height-100)
    top=max(40,int(center-height/2)); bottom=min(image.height-40,top+height)
    if bottom-top<height: top=max(40,bottom-height)
    margin=max(45,int(image.width*0.03))
    return [margin,top,image.width-margin,bottom]

def mask_text(mask,tokens,window):
    left,top,right,bottom=window
    for token in tokens:
        x0=token["left"]-left; y0=token["top"]-top
        x1=x0+token["width"]; y1=y0+token["height"]
        if x1<0 or y1<0 or x0>=right-left or y0>=bottom-top: continue
        pad=8
        cv2.rectangle(mask,(max(0,x0-pad),max(0,y0-pad)),(min(mask.shape[1]-1,x1+pad),min(mask.shape[0]-1,y1+pad)),0,-1)
    ...
    while changed:
        ...
        while boxes:
            ...
            for i,b in enumerate(boxes):
                ...
                h_overlap=max(0,min(ay2,by2)-max(ay,by))/max(1,min(ah,bh))
                v_overlap=max(0,min(ax2,bx2)-max(ax,bx))/max(1,min(aw,bw))
                ...
                if (h_overlap>0.25 and x_gap<max_gap) or (v_overlap>0.25 and y_gap<max_gap):
                    nx,ny=min(ax,bx),min(ay,by); nx2,ny2=max(ax2,bx2),max(ay2,by2)
                    boxes[i]=[nx,ny,nx2-nx,ny2-ny]; changed=True; merged=True; break
            if not merged: out.append(a)
        boxes=out
    return boxes

def detect_figure_bbox(page_image,tokens,match,target):
    window=local_window(page_image,match,target)
    left,top,right,bottom=window
    patch=np.asarray(page_image.crop(window)).copy()
    hsv=cv2.cvtColor(patch,cv2.COLOR_RGB2HSV)
    gray=cv2.cvtColor(patch,cv2.COLOR_RGB2GRAY)
    # Textbook diagrams are usually colored or have darker line art. Keep both, then erase OCR text.
    sat=hsv[:,:,1]; val=hsv[:,:,2]
    mask=((sat>28)|(gray<218)|(val<210)).astype(np.uint8)*255
    mask_text(mask,tokens,window)
    mask=cv2.morphologyEx(mask,cv2.MORPH_OPEN,cv2.getStructuringElement(cv2.MORPH_RECT,(3,3)))
    mask=cv2.morphologyEx(mask,cv2.MORPH_CLOSE,cv2.getStructuringElement(cv2.MORPH_RECT,(11,11)),iterations=2)
    mask=cv2.dilate(mask,cv2.getStructuringElement(cv2.MORPH_RECT,(7,7)),iterations=1)
    contours,_=cv2.findContours(mask,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ph,pw=mask.shape; boxes=[]
    for contour in contours:
        x,y,w,h=cv2.boundingRect(contour); area=w*h
        if w<120 or h<90 or area<pw*ph*0.006: continue
        if w>pw*0.94 and h>ph*0.82: continue
    ...
    # Prefer a substantial graphic near the target phrase, but penalize page-wide text/decor bands.
    target_y=(match["bbox"][1]+match["bbox"][3]/2+target.get("bias",0))-top
    scored=[]
    for box in boxes:
        x,y,w,h=box; area=(w*h)/(pw*ph); cy=y+h/2
        prox=abs(cy-target_y)/max(ph,1)
        edge_penalty=(0.25 if x<10 or x+w>pw-10 else 0)+(0.18 if y<10 or y+h>ph-10 else 0)
        skinny_penalty=0.3 if (w/h>6 or h/w>5) else 0
        score=area*3.6+(1-min(prox,1))*1.1-edge_penalty-skinny_penalty
        if w>=target.get("min_w",0) and h>=target.get("min_h",0): score+=0.35
        scored.append((score,box))
    scored.sort(reverse=True,key=lambda item:item[0])
    best=scored[0][1]
    x,y,w,h=best
    # If adjacent visual components form one textbook figure, include nearby large boxes.
    selected=[best]
    for _,box in scored[1:5]:
        bx,by,bw,bh=box
        if bw*bh<pw*ph*0.015: continue
        vertical_overlap=max(0,min(y+h,by+bh)-max(y,by))/max(1,min(h,bh))
        horizontal_overlap=max(0,min(x+w,bx+bw)-max(x,bx))/max(1,min(w,bw))
        gapx=max(0,max(x,bx)-min(x+w,bx+bw)); gapy=max(0,max(y,by)-min(y+h,by+bh))
        if (vertical_overlap>0.3 and gapx<pw*0.08) or (horizontal_overlap>0.3 and gapy<ph*0.08): selected.append(box)
    sx=min(b[0] for b in selected); sy=min(b[1] for b in selected)
    sx2=max(b[0]+b[2] for b in selected); sy2=max(b[1]+b[3] for b in selected)
    pad=max(22,int(min(pw,ph)*0.025))
    sx=max(0,sx-pad); sy=max(0,sy-pad); sx2=min(pw,sx2+pad); sy2=min(ph,sy2+pad)
    absolute=[left+sx,top+sy,sx2-sx,sy2-sy]
...
def crop_figure(page_image,tokens,match,target):
    ...
    if bbox is None:
        l,t,r,b=window; bbox=[l,t,r-l,b-t]
    x,y,w,h=bbox
    # Reject the old failure mode: nearly full-width page strips with lots of body text.
    if w>page_image.width*0.93 and h>page_image.height*0.36:
        raise RuntimeError(f"Unsafe page-strip crop {w}x{h}")
...
def make_contact_sheet(records):
    thumbs=[]
    for r in records:
        img=Image.open(OUT_DIR/f"{r['id']}.webp").convert("RGB")
        img.thumbnail((480,300))
        tile=Image.new("RGB",(500,350),"white"); tile.paste(img,((500-img.width)//2,30))
        draw=ImageDraw.Draw(tile); draw.text((10,8),f"{r['id']} | p{r['page']}",fill="black")
        thumbs.append(tile)
    cols=3; rows=math.ceil(len(thumbs)/cols)
    sheet=Image.new("RGB",(cols*500,rows*350),(235,235,235))
...
def main():
    ...
    for asset_id,target in TARGETS.items():
        ...
        payload=cache[hit["page_index"]]
        crop,bbox=crop_figure(payload["image"],payload["tokens"],hit,target)
    ...
    print(f"Extracted {len(records)} focused textbook figures ({unique} unique).")
