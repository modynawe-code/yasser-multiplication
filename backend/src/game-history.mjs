...
const FAMILY_SCOPE='default-family';
...
function terminalOutcome(gameId,state){
  ...
  if(gameId==='xo'){
    if(state.status==='won'&&state.winner)return{winnerIds:[state.winner],scores:{[state.winner]:1},details:{round:state.round||1,moveCount:state.moveCount||0}};
  ...
  if(gameId==='domino'){
    if(state.status==='finished'&&state.winner)return{winnerIds:[state.winner],scores:{},details:{round:state.round||1,roundTotals:state.roundTotals||{},blocked:Boolean(state.blocked)}};
    if(state.status==='draw')return{winnerIds:[],draw:true,scores:{},details:{round:state.round||1,roundTotals:state.roundTotals||{},blocked:Boolean(state.blocked)}};
    return null;
  }
...
export function normalizeGameMatch(input,{recordedAt=new Date().toISOString()}={}){
  ...
  if(!matchId||!GAME_ID.test(gameId)||!MODES.has(playMode))return{ok:false,error:'invalid_match'};
  ...
  const winnerIds=uniq(input?.winnerIds).filter(id=>playerIds.has(id));
  if((input?.winnerIds||[]).length!==winnerIds.length)return{ok:false,error:'invalid_winners'};
  const startedAt=isoOrNull(input?.startedAt),endedAt=isoOrNull(input?.endedAt)||recordedAt;
  return{ok:true,value:{
    matchId,familyScope:FAMILY_SCOPE,gameId,gameVersion,playMode,sourceRoomId:cleanText(input?.sourceRoomId,120)||null,
...
export async function persistGameMatch(env,match){
  const parsed=match?.matchId?{ok:true,value:match}:normalizeGameMatch(match);
  if(!parsed.ok)return parsed;
  const m=parsed.value;
  const existing=await env.DB.prepare('SELECT match_id,recorded_at FROM game_matches WHERE match_id=?').bind(m.matchId).first();
  if(existing)return{ok:true,duplicate:true,matchId:existing.match_id,recordedAt:existing.recorded_at};
  const statements=[
    env.DB.prepare('INSERT INTO game_matches(match_id,family_scope,game_id,game_version,play_mode,source_room_id,source_room_version,started_at,ended_at,recorded_at,winner_ids_json,details_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)')
      .bind(m.matchId,m.familyScope,m.gameId,m.gameVersion,m.playMode,m.sourceRoomId,m.sourceRoomVersion,m.startedAt,m.endedAt,m.recordedAt,JSON.stringify(m.winnerIds),m.detailsJson),
...
export async function recordOnlineRoomMatch(env,args){
  const match=buildOnlineRoomMatch(args);if(!match?.ok)return match?match:null;
  return persistGameMatch(env,match.value);
}

export async function createLocalGameHistory(request,env,respond,readJson){
  const recordedAt=new Date().toISOString(),parsed=normalizeGameMatch(await readJson(request),{recordedAt});
...
export async function listGameHistory(request,env,respond){
  const url=new URL(request.url),limit=Math.max(1,Math.min(100,Number(url.searchParams.get('limit')||30)||30));
  const rows=await env.DB.prepare('SELECT match_id,game_id,game_version,play_mode,started_at,ended_at,recorded_at,winner_ids_json FROM game_matches WHERE family_scope=? ORDER BY ended_at DESC LIMIT ?').bind(FAMILY_SCOPE,limit).all();
  const matches=[];
...
export async function gameHistoryStats(request,env,respond){
  ...
  const rows=await env.DB.prepare(`SELECT p.learner_id,p.display_name,
    ...
    WHERE m.family_scope=? AND m.ended_at>=?
    GROUP BY p.learner_id,p.display_name
    ORDER BY cup_points DESC,wins DESC,matches DESC,p.display_name`).bind(FAMILY_SCOPE,since).all();
