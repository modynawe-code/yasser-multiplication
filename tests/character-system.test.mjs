...
test('learning flow keeps visual feedback visible until the next question',async()=>{
  ...
  assert.match(controller,/visualHold=visuals\.render/);
  assert.match(controller,/setTimeout\(renderQuestion,\(visualHold\|\|fallbackHold\)\+80\)/);
  ...
  assert.match(scenes,/duration:1600/);
  assert.match(scenes,/duration:2200/);
